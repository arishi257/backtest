...
FYERS_CACHE_DIR = Path(__file__).resolve().parents[2] / ".backtest_data_cache" / "fyers_5s"
MIN_HEDGE_THRESHOLD_LOTS = 1.2
IST = ZoneInfo("Asia/Kolkata")
OPTION_RE_TEMPLATE = (
    r"^5s/{underlying}(?P<expiry>\d{{6}})(?P<strike>\d+)(?P<option_type>CE|PE)_"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run a separate 5-second FYERS headless backtest."
    )
    ...
    print(
        f"Completed {cycles} 5-second replay cycles across {len(sessions)} expiries. "
...
def load_fyers_dataset(
    ...
    timestamps = pd.date_range(
        ...
        pd.Timestamp(f"{trade_date} 15:24:55", tz=IST),
        freq="5s",
    )
...
def parse_single_run_output(output: str) -> dict[str, float | None]:
    ...
    for line in output.splitlines():
        completed_match = re.search(r"Completed\s+(\d+)\s+5-second replay cycles", line)
        if completed_match:
...
def run_single_backtest(
    ...
    dataset = load_fyers_dataset(options_zip, index_zip, trade_date, underlying)
    future_series = dataset.future_series
    if future_series is None:
        raise ValueError(f"{underlying} index OHLC was not loaded.")
    ...
    if not final_metrics:
        raise ValueError("No analytics rows were produced.")
...
def show_batch_results_gui(rows: list[dict[str, object]], args: argparse.Namespace) -> None:
    ...
    root = tk.Tk()
    root.title(f"{args.underlying} 5s Batch Results")
    ...
    title = ttk.Label(
        header,
        text=(
            f"{args.underlying} 5s batch results | "
    ...
    for column in columns:
        ...
        tree.column(
            ...
            minwidth=80,
            anchor=tk.E if column != "date" and column != "detail" else tk.W,
...
def print_batch_table(rows: list[dict[str, object]]) -> None:
    if not rows:
        print("No weekday dates found in the requested range.")
        return
    columns = batch_result_columns()
    string_rows = [
        [format_batch_value(column, row.get(column)) for column in columns]
        for row in rows
    ]
    widths = [
...
def load_option_frame(
    ...
    if not rows:
        raise ValueError(
            f"No 5s {normalized_underlying} option rows found for {trade_date} in {options_zip}."
        )
...
def load_index_series(
    ...
    member_name = f"5s/{normalized_underlying}_{trade_date:%Y_%m_%d}.json"
...
def open_option_archive(
    ...
    with zipfile.ZipFile(options_zip) as archive:
        direct_members = [
            ...
            if name.startswith("5s/") and name.endswith(".json")
        ]
        if direct_members:
            yield archive
            return
...
def option_cache_path(options_zip: Path, trade_date: date, underlying: str) -> Path:
    return (
        FYERS_CACHE_DIR
        / underlying
        / f"{options_zip.stem}_{trade_date:%Y%m%d}_options_5s.pkl"
    )
...
def index_cache_path(index_zip: Path, trade_date: date, underlying: str) -> Path:
    return (
        FYERS_CACHE_DIR
        / underlying
        / f"{index_zip.stem}_{trade_date:%Y%m%d}_index_5s.pkl"
    )
...
def legacy_option_cache_path(options_zip: Path, trade_date: date, underlying: str) -> Path:
    return (
        FYERS_CACHE_DIR
        / underlying
        / f"{options_zip.stem}_{trade_date:%Y%m%d}_options_5s.csv"
    )
...
def legacy_index_cache_path(index_zip: Path, trade_date: date, underlying: str) -> Path:
    return (
        FYERS_CACHE_DIR
        / underlying
        / f"{index_zip.stem}_{trade_date:%Y%m%d}_index_5s.csv"
    )
